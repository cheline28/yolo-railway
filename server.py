...
fps_history  = []
last_time    = time.time()
COLORS       = ["#56B4E9","#E69F00","#009E73","#D55E00","#0072B2"]
...
def draw_detections(img_pil, results):
    ...
    for r in results:
        for box in (r.boxes or []):
            ...
            draw.rectangle([x1, y1-16, x1+len(label)*7+44, y1], fill=color)
            draw.text((x1+2, y1-14), f"{label} {conf:.2f}", fill="white")
            detections.append({
                "label"     : label,
...
def detect():
    global last_time, latest_frame

    if model is None:
        return jsonify({"status": "loading", "message": "Model sedang dimuat, coba lagi"}), 503

    # Ambil data gambar — support raw JPEG maupun multipart
    try:
        ...
        return jsonify({"error": f"Gagal baca request: {str(e)}"}), 400
    ...
    try:
        ...
        return jsonify({"error": f"Gagal decode gambar: {str(e)}"}), 422
    ...
    buf = io.BytesIO()
    annotated.save(buf, format="JPEG", quality=75)
    with lock:
    ...
    def gen():
        while True:
            ...
            if frame is None:
                ...
                ImageDraw.Draw(img).text((50, 110), "Menunggu frame...", fill="gray")
                buf = io.BytesIO()
...
def index():
    ...
    .r{display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #333}
    </style>
    <script>
    async function r(){
    ...
    <img src="/stream">
    <div class="s">
        <div class="r"><span>Status</span>      <span id="st">-</span></div>
